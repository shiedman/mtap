...
function checkin(){
    ...
    for(var i=0;i<users.length;i++){
        ...
        if(Date.now()-info.ntime>=0 && info.pass){
...
function downloadDir(dirid){
    ...
    var site=new Site('y2be@163.com','ssskha201279',new HttpClient(true));
    site.on('login',function(content){ site.crosslogin(content); });
    site.on('ready',function(){
        site.listfiles(dirid,function(files){
            //var list=files.filter(function(e){return e.size>0;});
            //site.downloadFiles(list);
...
if(true){
    ...
    setTimeout(function(){
        try{
            ...
            //var site=new Site('y2be@163.com','ssskha201279',new HttpClient(true));
            //site.on('login',function(content){ site.crosslogin(content); });
            //site.on('ready',function(){console.log('good');});
            //site.login();
            downloadDir(0);
        }catch(err){
            console.log(err);
