...
function post(url,data,headers,res_callback,data_callback){
    ...
    if(filepath){var fstat=fs.statSync(filepath);filesize=fstat.size;}
    if(!headers)headers={};
    ...
    file.on('data',function(data){
        ...
        if(typeof(data_callback)=='function')data_callback(data);
    });
    ...
    if(file)req.on('drain',function(){ file.resume(); });
    return req;
}
exports.post=post;
