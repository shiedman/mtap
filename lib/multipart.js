...
function post(url,form,headers,res_callback,data_callback){
    ...
    function send(value){
        ...
        file.on('data',function(chunk){
            if(!req.writable){file.destroy();return;}
        ...
        file.on('close',function(){
            setTimeout(function(){file.removeAllListeners();},10000);
            //req.removeAllListeners('drain');
        });
    ...
    req.on('close',function(){
        ...
        //error maybe emit after request closed
        setTimeout(function(){req.removeAllListeners();},10000);
    });
    req.on('error',function(err){
        //if(uploading){
            //uploading=false;
            //if(file){file.destroy(); file.removeAllListeners();}
        if(res_callback){res_callback(err);res_callback=null;}
        console.error(err);
        //}
    });
    send(cache.shift());
