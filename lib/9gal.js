...
function login(username,passwd,callback){
    ...
    var payload=qs.stringify({
        'pwuser':username,'pwpwd':passwd,'jumpurl':'index.php','step':2,cktime:31536000
    });
...
function takeBonus(){
    var cfg=ut.ini.param('9gal'),user=cfg.user,pass=cfg.pass;
...
if(false){
    ...
    //login('y2be','su201279',function(success){
        //if(!success){console.warn('[9gal]%s login failed',username);return;}
        //fetchLink(function(link){
            //if(!link){console.warn('[9gal]%s:link is empty',username);return;}
            //clickAds(link);
        //});
    //});
    ut.Cookie.load();
