...
function checkin(){
    var cfg=ut.ini.param('bbs.9gal.com');
    //if (!cfg)return;
    if(!cfg.name || !cfg.pass)return;
    if(cfg.ntime && Date.now()-cfg.ntime<0)return;
    //if(cfg.ntime && Date.now()-cfg.ntime<_10mins)return;
    if(cfg.tried>5){
        cfg.ntime=Date.now()+2*_1hour;
        cfg.tries=0;
        ut.log('9gal.log','tried 5 times ,please check username and password');
        return;
    }

    var site=new Site(cfg.name,cfg.pass);
    site.on('login',function(){
        site.clickAds(function(msg){
            if(msg){
                logger.info(msg);
                ut.log('9gal.log',msg);
                cfg.ntime=Date.now()+5*_1hour;
                cfg.tries=0;
            }
        })
    });
    site.login();
    cfg.tried=cfg.tried||0;
    cfg.tried++;
}
//exports.Site=Site;
