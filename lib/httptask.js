...
function queueUpload(func,filepath){
    ...
    for(var i=0;i<uploadQueue.length;i++){
        var u=uploadQueue[i];
        if(u.func==func && u.filepath==filepath){
            logger.warn('func[%s] is uploading: %s',func.name,filepath);
            return;
        }
    }
    uploadQueue.push({func:func,filepath:filepath});
    return true;
}
/**
 * func:function object
 * params[0]: download url
 * params[1]: option settting
 *            headers:['k1:v1',['k2:v2']
 *            output: save filename
 */
function queueDownload(func,params){
    if(!params||params.length<1){throw new Error('url is missing');}
    for(var i=0;i<downloadQueue.length;i++){
        var d=downloadQueue[i];
        if(d.func==func && d.params[0]==params[0]){
            logger.warn('currently downloading : %s',params[0]);
            return;
        }
    }
    downloadQueue.push({func:func,params:params});
    return true;
}
...
Task.prototype.pause=function(){
    ...
    logger.log('task[%s] paused:%s',this.id,this.file.name);
...
function updateTask(){
    ...
    while (n--){
        ...
        var timeElapse=Date.now()-t.info.time;
        // more than 30s,no data transfer between remote server
        if (t.status>0 && timeElapse > 30000){
            t.abort();
        ...
        if (t.status<=0 && timeElapse>43200000){
            logger.info('clear task:%s',t.file.name);
            tasklist.splice(n,1);
        }
        //more than 10mins, release any resources held
        if (t.status<=0 && timeElapse>300000 && t._emitter){
            if(t._emitter){
                logger.info('release resource: %s',t.file.name);
                t._emitter.removeAllListeners();
                delete t._emitter;
                delete t.resume;
            }
        }
    ...
    if(uploading==0&&uploadQueue.length>0){
        var up=uploadQueue.shift();
        try{
            up.func.call(null,up.filepath)
        }catch(err){
    ...
    if(downloading<2&&downloadQueue.length>0){
        var dl=downloadQueue.shift();
        try{
            logger.info('[download queue] %j',dl.params[0]);
            dl.func.apply(null,dl.params);
        }catch(err){
            console.error(err.message);
            console.error(err.stack);
        }
