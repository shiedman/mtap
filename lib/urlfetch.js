...
HttpClient.prototype.request=function(method,url,payload,_headers,callback){
    ...
    self.build_header(method,url,_headers,payload,function(headers){
        self._request(method,url,payload,headers,function(err,res){
            ...
            //------------callback-------------------
            if(res.statusCode==302 && res.headers['location'] && self.redirect>0){
                self.redirect--;
...
HttpClient.prototype._request=function(method,requestURL,payload,headers,callback){
    ...
    var req=conn.request(options,function(res){
        if(res.statusCode==302)return callback(null,res);//redirect
...
var cookieStore={
    ...
    save:function(file){
        ...
        console.log('%s - [cookie]saved: %s',ut.datetime(),file);
    },
    load:function(file){
        if(!file)file=this.file;
        if(!fs.existsSync(file)){logger.error('[cookie]file not exists:%s',file);return;}
        this.file=file;
        fs.readFile(file,'utf-8',function(err,data){
            ...
            console.log('%s - [cookie]loaded: %s',ut.datetime(),file);
            }catch(err){
...
if(process.env.PORT_PROXY){
    ...
    setInterval(function(){ cookieStore.save(); },1800000);
}
...
if(false){
    ...
    sess.get('http://bbs.9gal.com',function(err,res){
