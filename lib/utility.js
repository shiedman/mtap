...
var _aria2={
    ...
    addUri:function(downloadURL,options,callback){
        ...
        jsonRPC(this.url,'aria2.addUri',params,callback||function(){});
    }
};
//check if aria2c available
//http.get(_aria2.url).on('error',function(err){_aria2.online=false;});
...
function jsonRPC(endPoint,method,params,callback){
    ...
    var req = http.request(options, function(res) {
        ...
        res.on('end',function(){
            var data=Buffer.concat(buff).toString();
            try{
...
function log(msg){
    ...
    fs.appendFile(path.join(__dirname,'../utility.log'),new Date()+' - '+msg+'\r\n');
}
