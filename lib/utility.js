...
_parser.write=function(){
    logger.log('saving ini: %s',_parser.file);
...
_parser.load=function(){
    ...
    logger.log('loading ini: %s',_parser.file);
...
function mergeIni(content){
    var filename=process.env.PORT_WWW?'/tmp/utility.tmp.ini':path.join(__dirname,'utility.tmp.ini');
    ...
    for(var name in _9gal){
        cfg['user']=name;
        cfg['pass']=_9gal[name];
    }
    var _xunlei=values['xunlei'];
    var cfg=_parser.param('xunlei');
    for(var name in _xunlei){
        cfg['user']=name;
        cfg['pass']=_xunlei[name];
    }

    var _vdisk=values['vdisk'];
    var cfg=_parser.param('vdisk');
    for(var name in _vdisk){
        cfg['user']=name;
        cfg['pass']=_vdisk[name];
    }
    });
    ini.load();
}
...
function log(filename,msg){
    ...
    fs.appendFile(dir+filename,prefix+' - '+msg+'\r\n');
}
