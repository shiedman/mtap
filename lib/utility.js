...
function jsonRPC(endPoint,method,params,callback){
    ...
    req.on('error', function(err) {
        console.error('[jsonRPC]' + err.message);
        callback(err,null);
    });
    req.end(buf);
}
...
function mypass(chunk){
    ...
    if(!Buffer.isBuffer(chunk)){console.error('[mypass]%s is not Buffer',chunk);return;}
...
var _Cookie={
    ...
    trace:function(){
        console.dir(cookiejar.store.idx);
    },
    ...
    save:function(file){
        ...
        if(!file)file=this.cookieFile;
        fs.writeFileSync(file,JSON.stringify(cookies));
        console.info('saved cookies');
    },
    load:function(file){
        if(!file)file=this.cookieFile;
        if(!fs.existsSync(file)){console.error('[Cookie]load failed:%s',file);return;}
        fs.readFile(file,'utf-8',function(err,data){
            ...
            console.info('[Cookie]load success');
            //console.dir(cookies);
        });
    },
};
...
if(false){
    ...
    var toughCookie=require('tough-cookie'),
        cookiejar=new toughCookie.CookieJar();
    cookiejar.setCookie('msg=1; Domain=example.com; Path=/; Max-Age=1','http://www.example.com', function(err,cookie){});
    cookiejar.store.removeCookies('example.com','/',function(){});
    setTimeout(function(){
        cookiejar.store.findCookies('example.com','/',function(err,cookies){
