...
Site.prototype.iterateFiles=function(nodes){
    var files=nodes||[],working=[],running=0,scans=0,self=this;
    function visitFiles(){
        if(files.length){
            //var d=dirs.shift();
            var d=files.shift();
            ...
            //console.log('running:%s',running);
            if(running==0){
                logger.info('scaned files: %s',scans);
            }
        }
    }
    visitFiles();
...
if(false){
    ...
    try{
        ...
        scan('shiedman@example.com','mypass','298327498','5bc864c8-f92b-4279-a079-79afc8d21f3b');
