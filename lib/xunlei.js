...
var webUploadRequest={
    ...
    info:function(transactionid,nodeid,filesize){
        var cfg={};
        var cookies=addFileRequest.headers['cookie'];
        ...
        cfg['nodeid']=nodeid;
        cfg['size']=filesize;
        var s="userid={luserid};sessionid={lsessionid};transactionid={transactionid};encrypttype=0;iscompress=0;keytype=0;chunksize_0={size};chunkseq_0=0;nodeid_0={nodeid};filesize_0={size};hash_0=0;";
        return s.format(cfg);
    },
    time:Date.now()
};
...
function uploadFile(cfg){
        ...
        var req=multipart.post(cfg._upload_url,data,headers,
                function(err,res){
                    ...
                    if(res)util.log('[upload]response: '+res.statusCode+' ==>'+filename);
