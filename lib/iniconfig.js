...
IniConfig.prototype.load = IniConfig.prototype.init = function load(file) {
    ...
    try {
        var self=this;
        fs.readFile(this.file, 'utf-8',function fileReadCb(err, data) {
            if (err) { throw err; }
            self.parse(data);
            console.log('%s - [iniconfig]loaded: %s',datetime(),self.file);
        });
    } catch (e) {
        console.error(e);
    }
...
IniConfig.prototype.write = function (file, le) {
    ...
    le = getLe(le);
     ...
     // create a headline
    var output = '; IniConfig' + le + '; ' +ut.datetime()+le;
...
if(false){
    var ini=new IniConfig('demo.ini');
    console.log(ini.file);
    ini.load();
    setTimeout(function(){
    //console.log(util.inspect(ini.section));

    console.log(ini.param('uptobox.com'));
    //ini.write('test.ini');
    },1000);
}
...
function datetime(){
    ...
    if(m<10)m='0'+m;
    if(s<10)s='0'+s;
    return d.getUTCFullYear()+'-'+(d.getUTCMonth()+1)+'-'+d.getUTCDate()+' '+h+':'+m+':'+s;
}
