...
logger.info('[vdisk]%s is login...',username);
...
ut.http.get(url,{'Referer':'http://vdisk.weibo.com/'},function(err,res){
    ...
    ut.http.post('http://login.sina.com.cn/sso/login.php?entry=weipan',
        ...
        var urls=[];
        var str=res.data;
        for(var i=str.indexOf('http:',str.indexOf('<script>')),j=-1;
            i>=0;i=str.indexOf('http:',j)){
        ...
        setTimeout(function(){ut.http.get(urls[2]);},2*3000);
        setTimeout(function(){
